...
exports.create = function(req,res,next){
	...
	var ap = new AP ({AP_name:ap_name,x:req.body.x,y:req.body.y,User:req.user.Username,Map:req.session.map});
	try {
...
exports.edit = function(req,res,next){
	...
	console.log(newname);
	try {
		AP.findOneAndUpdate({AP_name:oldname,User:req.user.Username,Map:req.session.map},{AP_name:newname},function(err,user){
			if(err){
				return next(err);
			}else{
				console.log(user);
